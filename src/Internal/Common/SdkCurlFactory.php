<?php
	...
	class SdkCurlFactory implements CurlFactoryInterface
	{
		...
		private function applyBody ( RequestInterface $request , array $options , array &$conf ) {
			$size = $request->hasHeader( 'Content-Length' ) ? (int) $request->getHeaderLine( 'Content-Length' ) : $request->getBody()
			                                                                                                              ->getSize();

			if ( $request->getBody()
			             ->getSize() === $size && $request->getBody()
			                                              ->tell() <= 0
			) {
		...
		private function applyHandlerOptions ( EasyHandle $easy , array &$conf ) {
			...
			if ( isset( $options['proxy'] ) ) {
				if ( !is_array( $options['proxy'] ) ) {
					...
					$scheme = $easy->request->getUri()
					                        ->getScheme();
					if ( isset( $options['proxy'][$scheme] ) ) {
						$host = $easy->request->getUri()
						                      ->getHost();
